...
def main(
    ...
    if po_token is None and cookies_path is not None:
        # Check if automatic PO token provider is available
        import requests
        try:
            response = requests.get("http://127.0.0.1:4416/ping", timeout=1)
            if response.status_code == 200:
                logger.info("Using automatic PO token provider")
            else:
                logger.warning("PO token provider not responding, downloading may fail")
        except:
            logger.warning("PO token provider not available, downloading may fail")
